...
def report_to_summary(report: AnalysisReport) -> str:
    high_risks = [risk for risk in report.risks if risk.level in ("high", "critical")]
    error_findings = [finding for finding in report.findings if finding.severity == "error"]
    decision = "BLOCK" if error_findings else "REVIEW" if high_risks or report.findings else "READY"
    lines: List[str] = [
        "# MCP Policy Forge Summary",
        "",
        "- Decision: `%s`" % decision,
        "- Tools: %s" % len(report.tools),
        "- Permission needs: %s" % len(report.needs),
        "- High or critical risks: %s" % len(high_risks),
    ...
    if high_risks:
        for risk in high_risks[:8]:
            lines.append("- `%s`: %s/%s - %s" % (risk.tool, risk.score, risk.level, "; ".join(risk.reasons) or "review required"))
        if len(high_risks) > 8:
            lines.append("- ... %s more high-risk tools" % (len(high_risks) - 8))
    else:
        lines.append("- No high or critical tool risks.")
    lines.extend(["", "## Findings", ""])
    if report.findings:
        for finding in report.findings[:10]:
            tool = " `%s`" % finding.tool if finding.tool else ""
            lines.append("- [%s] `%s`%s: %s" % (finding.severity, finding.code, tool, finding.message))
        if len(report.findings) > 10:
            lines.append("- ... %s more findings" % (len(report.findings) - 10))
    else:
        lines.append("- No policy findings.")
    lines.extend(["", "<details>", "<summary>Generated least-privilege policy</summary>", "", "```json"])
    import json

    lines.append(json.dumps(report.policy.to_dict(), ensure_ascii=False, indent=2, sort_keys=True))
    lines.extend(["```", "", "</details>", "", "<!-- generated by mcp-policy-forge -->"])
    return "\n".join(lines) + "\n"
